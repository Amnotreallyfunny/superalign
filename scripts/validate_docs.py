        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if "ModuleNotFoundError" in result.stderr:
                     print(f"FAILED: block {i+1} failed due to missing module.\n{result.stderr}")
                     sys.exit(1)
                else:
                    # If it's an error about missing files, we might ignore it 
                    # if the goal is just syntactic/import correctness.
                    print(f"Warning: block {i+1} executed with return code {result.returncode}. This might be expected if data files are missing.")
            else:
