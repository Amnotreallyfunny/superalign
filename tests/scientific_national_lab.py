...
def compute_directory_hash(path: str) -> str:
...
def run_brutal_reconciliation_test() -> None:
...
def run_scale_memory_test(count: int = 100000) -> None:
    ...
    data = [
        {
            ...
            "raw_label": f"Exp_Taxon_{i}_{uuid.uuid4().hex[:8]}",
        }
    ...
    print(
        f"Memory Delta: {final_mem - mid_mem:.2f} MB | "
...
def run_distributed_determinism_test() -> None:
    ...
    for t_id, loc_name, seq_str in [
        (t, loc, "A" * length) for t in taxa for loc, length in loci
    ]:
        engine_ref.write_taxon_locus(plan_ref, t_id, loc_name, seq_str.encode())

    ref_hash = compute_directory_hash(ref_store)
    assert (
        distributed_hash == ref_hash
    ), "State Divergence: Parallel write corrupted the matrix."
